...
def fold_input(in_file):
  ...
  #print len(parse_input(lines[0].strip()))
  for i in range(len(lines)-1):
    #print len(parse_input(lines[i+1].strip()))
...
if firstrun == "N":
  f = open(LastPos)
  #previouspos = np.genfromtxt(StringIO(f), delimiter=" ")
  previouspos = []
  for l in f:
    prepos = np.genfromtxt(StringIO(l), delimiter=" ")
    n = len(prepos)
    prepospos = []
    for i in range(n):
      prepospos.append(prepos[i])
    previouspos.append(prepospos)
  print previouspos

if withpsr == "Y":
  sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob_psr, args=(coords_ref,Nm_ref,coords_psr,Nm_epc),threads=6)
else:
  sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=(coords_ref,Nm_ref),threads=6)
...
for k in range(100):
 if firstrun == "N":
   result = sampler.run_mcmc(previouspos, nsteps_per)
   previouspos=result[0]
 elif firstrun == "Y":
...
for j in range(nwalkers):
    ax1.errorbar(steps,sampler.chain[j,:,0])
ax2 = plt.subplot(612, sharex=ax1) 
for j in range(nwalkers):
    ax2.errorbar(steps,sampler.chain[j,:,1])  
ax3 = plt.subplot(613, sharex=ax1) 
for j in range(nwalkers):
    ax3.errorbar(steps,sampler.chain[j,:,2])
ax4 = plt.subplot(614, sharex=ax1) 
for j in range(nwalkers):
    ax4.errorbar(steps,sampler.chain[j,:,3]) 
ax5 = plt.subplot(615, sharex=ax1) 
for j in range(nwalkers):
    ax5.errorbar(steps,sampler.chain[j,:,4]) 
ax6 = plt.subplot(616, sharex=ax1)  
for j in range(nwalkers):
    ax6.errorbar(steps,sampler.chain[j,:,5]) 
xticklabels1 = ax1.get_xticklabels()
setp(xticklabels1, visible=False) 
xticklabels2 = ax2.get_xticklabels()
setp(xticklabels2, visible=False) 
xticklabels3 = ax3.get_xticklabels()
setp(xticklabels3, visible=False) 
xticklabels4 = ax4.get_xticklabels()
setp(xticklabels4, visible=False)
xticklabels5 = ax5.get_xticklabels()
setp(xticklabels5, visible=False)
plt.show()
