...
SampleName = imgs+"_"+"RefEpoch"+"_"+"%1i"% (nref)+"_"+"Thread"+"_"+"%1i"% (thread_num)+".chain"
LastPos = imgs+"_"+"RefEpoch"+"_"+"%1i"% (nref)+"_"+"LastPos"+".chain"
...
def lnprob(th):
    ...
    for i in range(nim):
        ...
        prob[i] = ((rotdelx.real/crdsref[i,:,2])**2 + (rotdelx.imag/crdsref[i,:,3])**2).sum()
    return -0.5*prob.sum()
...
for i in range(nim-1):
    ...
    p0.append(1.)
for j in range(nst):
    p0.append(crdsref[nref,j,0]*(0.995+1e-2*np.random.rand()))
    p0.append(crdsref[nref,j,1]*(0.995+1e-2*np.random.rand()))
p0.append(crdspsr[nref,0,0]*(0.995+1e-2*np.random.rand()))
p0.append(crdspsr[nref,0,1]*(0.995+1e-2*np.random.rand()))
p0.append(0.)
p0.append(0.)
...
for i in range(nPlot):
    for j in range(nwalkers):
        ax[i].errorbar(steps,sampler.chain[j,:,i+ndim-nPlot])
setp([a.get_xticklabels() for a in ax[:nPlot-1]], visible=False)
#plt.show()
plt.savefig(imgs+"_"+"RefEpoch"+"_"+"%1i"% (nref)+".jpg")
